...
class UI():
    def __init__(self, name):
        ...
        self.networkGameImage = pygame.image.load(data.NETWORKGAME).convert_alpha()
    ...
    def displayPlay(self):
        ...
        for text in sideText:
            self.sideSurface = self.msgFont.render(text, True, (102, 204, 255), (255, 255, 255))
    ...
    def displayQuery(self):
        ...
        self.text_surface = self.queryFont.render(self.game.msg, True, (102, 204, 255), (255, 255, 255))
    ...
    def setScreen(self):
        if self.game.fullScreenModChanged:
            self.screen = pygame.display.set_mode((data.SCREEN_WIDTH, data.SCREEN_HEIGHT), self.game.fullScreenMod, 32)
    ...
    def fixXY(self, target):
        x = self.game.gameMap[target][0] * data.SCREEN_WIDTH - data.CHESSMAN_WIDTH*0.5
        y = self.game.gameMap[target][1] * data.SCREEN_HEIGHT - data.CHESSMAN_HEIGHT*1
        return (x, y)
